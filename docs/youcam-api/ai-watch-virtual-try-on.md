...
- **Source:** [OpenAPI description](https://docs.perfectcorp.com/_bundle/reference/ai_watch.yaml)
...
| `object_infos.parameter.watch_need_remove_background` | `boolean` | No | Whether the watch image needs background removal. |
| `object_infos.parameter.watch_anchor_point` | `array<array<integer>>` | No | First 2 points mark the strap end when worn. Remaining 2 points mark the upper & lower edges of watch case. <br>Type: 4x2 list of integers <br>Range: 4 (x, y) pixel coordinates <br>Default: null (use engine default) |
| `object_infos.parameter.watch_wearing_location` | `number` | No | Defines the placement of the watch on the wrist in the user photo. <br>Range: -0.3 ~ 1.0 (0 = closest to the wrist joint) <br>       (smaller number = placed close to the palm <br>       larger number = placed further away from the wrist) <br>Default: null (use engine default) |
| `object_infos.parameter.watch_shadow_intensity` | `number` | No | Shadow intensity applied to the watch (higher = darker shadow). |
| `object_infos.parameter.watch_ambient_light_intensity` | `number` | No | Ambient light adjustment intensity for the watch. |
