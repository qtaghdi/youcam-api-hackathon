<!-- Generated by tools/youcam-docs-sync. Do not edit this file directly. -->

# AI Earrings Virtual Try On

## Overview
The Ultimate AI Earring Virtual Try-On
Top AI ear piercing simulator for virtual earring try-on and virtual piercing try-on

Create realistic and dynamic earrings vitual try-on from a 2D image, no expensive 3D modelling required. Our advanced algorithms create lifelike virtual try-on earring SKUs with sophisticated lighting effects and physically accurate motions.

### Integration Guide
This guide walks you through:

*   **Endpoint:** `/s2s/v2.0/file/2d-vto/earring`
*   **Authentication:** All requests require an `Authorization: Bearer YOUR_API_KEY`
*   **Workflow:**
    1.  **Prepare a selfie image:** Uploading an image or provide a valid image URL
    2.  **Prepare an earring image:** Uploading an image or provide a valid image URL of an earring product
    3.  **Fire an AI task and Retrieve Task ID:** Capture the `task_id` from the response.
    4.  **Poll Status (`GET`):** Use the `task_id` to check the status of the task. Continue polling until `task_status` is `"success"` or `"error"`.

---

* API Playground

Interactively explore and test the API using our official playground:

**API Playground:**
[http://yce.makeupar.com/api-console/en/api-playground/ai-earring-virtual-try-on/](http://yce.makeupar.com/api-console/en/api-playground/ai-earring-virtual-try-on/)

---

* Authentication
- Include your API key in the request header using **Bearer Token**:
    ```
    Authorization: Bearer YOUR_API_KEY
    ```
You can find your API Key at https://yce.makeupar.com/api-console/en/api-keys/.


* 1. Upload an Image

You may upload a file directly to the server or provide a valid image URL in the VTO task payload.

   * Upload Endpoint

```
POST /s2s/v2.0/file/2d-vto/earring
```

Alternatively, skip this step if you already have a public image URL.

You may upload a file directly to the URL provided in the response from the File API and then use the corresponding `src_file_id` returned by the File API to invoke the AI task later. Or provide a valid image URL in the VTO task payload as `src_file_url`. The `src_file_id` or `src_file_url` will serve as the virtual try-on target.

You must also provide another earring product image as a reference using `ref_file_ids` or `ref_file_urls` to be applied to your `src_file_id` or `src_file_url`.

The AI engine supports automatic background removal for your earring product image. However, you may provide an occlusion mask image file for either your hand (`srcmsk_file_id` or `srcmsk_file_url`) or the earring product (`refmsk_file_ids` or `refmsk_file_urls`) to fine-tune the segmentation.

---

* 2. Create a Earring VTO Task and Poll for Results

Once you have an image and a template ID, create a task. The API processes the request asynchronously. You must poll the task status until it reaches `success` or `error`.

   * Create Task Endpoint

```
POST /s2s/v2.0/task/2d-vto/earring
```

   * Polling Endpoint

```
GET /s2s/v2.0/task/2d-vto/earring/{task_id}
```

---

### File Specs & Errors

* AI Earring Virtual Try-On Specification

**Supported Earring Reference Image**
* A single earring image in a clear front view without obstruction.
*   All parameters (including anchor points, masks, location, etc.) apply **only** when the reference image shows a **single earring** being worn.
*   If the try-on reference image shows **both earrings**, all parameters will use **auto-detection and default settings**.
*   When trying on **both earrings**, the clearer ear will be used as the source, and the other ear will be generated by mirroring it.

![](https://bcw-media.s3.ap-northeast-1.amazonaws.com/strapi/assets/earring_product_01_41c943f9fc_037ffb1241.jpg) ![](https://bcw-media.s3.ap-northeast-1.amazonaws.com/strapi/assets/earring_product_07_5476e0a156_a69e8e6549.jpg)

**Supported Selfie View**

*   The AI Earring Virtual Try-On supports front-facing images, but the best results are achieved with side-facing images.

![](https://bcw-media.s3.ap-northeast-1.amazonaws.com/strapi/assets/Earring_restriction_cdf1de3c7b.png)

**earring\_wearing\_location: integer array of size 2**
Specifies the target location in the selfie where the earring should be placed.
Default value: null (engine default)

![](https://bcw-media.s3.ap-northeast-1.amazonaws.com/strapi/assets/wearing_location_b4f6f4453a.jpg)

**earring\_scale: number greater than 0**
Controls the earring size in centimetres.
Default value: null (engine default)

**earring\_is\_right\_ear: boolean**
Indicates whether the earring is worn on the right ear. By default, it is worn on the right ear.
Default value: true

**earring\_occluded\_type: number (Enum: 0, 1, 2)**
Specifies the occlusion type:
0 means auto-detect
1 means occluded
2 means no occlusion
Default value: 0

**earring\_shadow\_intensity: float (0.0 to 1.0)**
Controls the shadow strength:
0.0 represents no shadow
1.0 represents maximum shadow
Default value: 0.15

**earring\_ambient\_light\_intensity: float (0.0 to 1.0)**
Defines how much the lighting references the selfie image:
0.0 ignores the selfie image lighting
1.0 fully matches the selfie image lighting and shadow rendering
Default value: 1.0

**earring\_anchor\_point: array of one point in pixel coordinate (optional)**
Specifies the wearing position in the earring product image.
Default value: null (engine default)

![](https://bcw-media.s3.ap-northeast-1.amazonaws.com/strapi/assets/anchor_point_787282aa19.jpg)


---

* Supported Formats & Dimensions

|AI Feature|Supported Dimensions|Supported File Size|Supported Formats|
|  ----  | ----  | ----  | ----  |
|AI Earring Virtual Try-On|long side <= 4096 |< 10MB|jpg/jpeg/png|

* Error Codes

|Error Code|Description|
|  ----  | ----  |
| RUNTIME_ERROR | An unexpected error occurred duearring runtime |
| PHOTO_DETECTION_FAIL | The user photo could not be processed correctly, for example no hand detected |
| OBJECT_DETECTION_FAIL | The object photo could not be processed correctly, for example no product detected |
| PHOTO_CHECK_INVALID | The pose or size of the user photo is invalid |
| INPUT_ERROR | The input file format is incorrect |
| INPUT_MAIN_IMAGE_EMPTY | A user image is required |


* Environment & Dependency

| Sample Code Language / Tool | Recommended Runtime Versions |
|---|---|
| cURL | - bash >= 3.2</br>   - curl >= 7.58 (modern TLS/HTTP support)</br>   - jq >= 1.6 (robust JSON parsing) |
| Node.js (JavaScript) | Node >= 18 (for global fetch) |
| JavaScript | - Chrome / Edge >= 80</br>   - Firefox >= 74</br>   - Safari >= 13.1 |
| PHP | PHP >= 7.4 (for modern TLS/compat), ext-curl (recommended) or allow_url_fopen=On + ext-openssl, ext-json |
| Python | Python >= 3.10 (for f-strings), requests >= 2.20.0 |
| Java | Java 11+ (for HttpClient), Jackson Databind >= 2.12.0 |

---

### JS Camera Kit
{% partial file="/_partials/js-camera-kit.md" /%}

## Reference

- **API version:** V1.0
- **OpenAPI version:** 3.0.0
- **Source:** [OpenAPI description](https://docs.perfectcorp.com/_bundle/reference/ai_earrings.yaml)
- **Servers:** `https://yce-api-01.makeupar.com`

## Authentication

| Name | Type | Scheme / Location | Description |
| --- | --- | --- | --- |
| BearerAuthenticationV2 | http | bearer | Use the standard 'Bearer authentication'. Put your 'API Key' in header: `Authorization:Bearer YOUR_API_KEY`. Notice that there is ' ' a space between 'Bearer' and the 'YOUR_API_KEY'. |

## Endpoints

### `POST` `/s2s/v2.0/file/2d-vto/earring`
**Create a new file.**

To upload a new file, you'll first need to use the File API. It will give you a URL – use that URL to upload your file. Once the upload is finished, you can use the file_id from the same response to start using our AI features.

**Tags:** V1.0

**Authentication:** BearerAuthenticationV2
#### Request body

**application/json** · required

**Schema:** `FileV1.1`

This object represents a file.

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `files` | `array<object>` | Yes | — |
| `files.content_type` | `string` | Yes | Content MIME type for this file. Currently available values are listed in the enum. |
| `files.file_name` | `string` | Yes | Name of this file |
| `files.file_size` | `integer` | Yes | Content length for this file in bytes. Should not be larger than 10MB. |

#### Responses

**200** — Successful retrieval of file upload information

Content types: `application/json`

**Schema:** `BasicFileResponse`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `files` | `array<object>` | No | — |
| `files.content_type` | `string` | No | Content MIME type for this file. |
| `files.file_name` | `string` | No | Name of this file |
| `files.file_id` | `string` | No | ID of this file. Other run task API will need this \`file_id\`. |
| `files.requests` | `array<object>` | No | Using upload \`url\`, \`headers\`, \`method\` below to upload file. After completion, the \`file_id\` is used to proceed with calling run task API. |
| `files.requests.headers` | `object` | No | Headers to include when uploading the file |
| `files.requests.url` | `string` | No | URL to upload this file |
| `files.requests.method` | `string` | No | HTTP method to upload this file |

**400** — Invalid request parameters

Content types: `application/json`

**Schema:** `object`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `error` | `string` | No | — |
| `error_code` | `string` | No | — |

**401** — Invalid or missing API key

Content types: `application/json`

**Schema:** `object`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `error` | `string` | No | — |

### `POST` `/s2s/v2.0/task/2d-vto/earring`
**Run an AI 2D Virtual Try On Earring task.**

This endpoint initiates the earring virtual try-on process. You must provide source file(s) and reference image(s) (via URL or File ID), along with specific parameters for alignment, shadowing, and wearing location. The task will be processed asynchronously, and you can check its status using the task_id returned in this response.

**Tags:** V1.0

**Authentication:** BearerAuthenticationV2
#### Request body

**application/json** · required

**Schema:** `object & BasicRunTaskV2SrcFileUrl & 2dVtoRunTaskV2SrcmskFileUrl & 2dVtoRunTaskV2RefFileUrls & 2dVtoRunTaskV2RefmskFileUrls | object & BasicRunTaskV2SrcFileUrl & 2dVtoRunTaskV2SrcmskFileUrl & 2dVtoRunTaskV2RefFileIds & 2dVtoRunTaskV2RefmskFileIds | object & BasicRunTaskV2SrcFileId & 2dVtoRunTaskV2SrcmskFileId & 2dVtoRunTaskV2RefFileUrls & 2dVtoRunTaskV2RefmskFileUrls | object & BasicRunTaskV2SrcFileId & 2dVtoRunTaskV2SrcmskFileId & 2dVtoRunTaskV2RefFileIds & 2dVtoRunTaskV2RefmskFileIds & 2dVtoRunTaskV2SrcInfo & 2dVtoRunTaskV2ObjInfos & object`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `src_file_url` | `string` | Yes | Url of the file to run task. The url should be publicly accessible. |
| `srcmsk_file_url` | `string` | No | Url of the source mask file to run task. The url should be publicly accessible. |
| `ref_file_urls` | `array<string>` | Yes | Url of the reference file to run task. The url should be publicly accessible. |
| `refmsk_file_urls` | `array<string>` | No | Url of the reference mask file to run task. The url should be publicly accessible. |
| `ref_file_ids` | `array<string>` | Yes | ID of the reference file file to run task. File ID from upload file API. |
| `refmsk_file_ids` | `array<string>` | No | ID of the reference mask file file to run task. File ID from upload file API. |
| `src_file_id` | `string` | Yes | ID of file to run task. File ID from upload file API. |
| `srcmsk_file_id` | `string` | No | ID of the source mask file to run task. File ID from upload file API. |
| `source_info` | `object` | Yes | Contain source photo informations. |
| `source_info.name` | `string` | Yes | The source url or File ID. |
| `source_info.mask_name` | `string` | No | The source mask url or File ID. |
| `object_infos` | `array<object>` | No | — |
| `object_infos.parameter` | `object` | No | — |
| `object_infos.parameter.earring_need_remove_background` | `boolean` | No | Whether the earring image needs background removal. |
| `object_infos.parameter.earring_anchor_point` | `array<integer>` | No | The anchor point of the earring image, used for alignment. <br>Type: list of 2 integers <br>Range: [x, y] (pixel coordinates) <br>Default: null (use engine default) |
| `object_infos.parameter.earring_wearing_location` | `array<integer>` | No | The target location of photo where the earring should be placed. <br>Type: list of 2 integers <br>Range: [x, y] (pixel coordinates) <br>Default: null (use engine default) |
| `object_infos.parameter.earring_scale` | `number` | No | Controls the earring’s size in centimeters. <br>Default: null (use engine default) |
| `object_infos.parameter.earring_is_right_ear` | `boolean` | No | Whether the earring is worn in right ear, defaultly is worn in right ear. |
| `object_infos.parameter.earring_occluded_type` | `number — 0 \| 1 \| 2` | No | Which occluded type the earring is. 0 means auto-detect, 1 means occluded type, 2 means no. |
| `object_infos.parameter.earring_shadow_intensity` | `number` | No | Shadow intensity applied to the earring (higher = darker shadow). |
| `object_infos.parameter.earring_ambient_light_intensity` | `number` | No | Ambient light adjustment intensity for the earring. |

#### Responses

**200** — Successful execution of the task

Content types: `application/json`

**Schema:** `BasicRunTaskResponseV2`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `data` | `object` | No | — |
| `data.task_id` | `string` | No | ID of this task. Task result is valid to query by this ID for 24 hours. |

**400** — Failed execution of task

Content types: `application/json`

**Schema:** `RunError`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `error` | `string` | No | Error message |
| `error_code` | `string — InvalidParameters \| CreditInsufficiency \| InvalidStyleGroup \| InvalidStyle \| BadRequest` | No | Error code:   * InvalidParameters - Invalid request parameters   * CreditInsufficiency - Insufficient unit to run   * BadRequest - Unexpected request parameter   * InvalidStyleGroup - Invalid style group id   * InvalidStyle - Invalid style id |

**401** — Invalid or missing API key

Content types: `application/json`

**Schema:** `object`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `error` | `string` | No | — |

**429** — Too many requests

Content types: `application/json`

**Schema:** `object`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `error` | `string` | No | — |

### `GET` `/s2s/v2.0/task/2d-vto/earring/{task_id}`
**Check the status of a AI 2D Virtual Try On Earring task.**

**Tags:** V1.0

**Authentication:** BearerAuthenticationV2
#### Parameters

| Name | In | Type | Required | Description |
| --- | --- | --- | --- | --- |
| `task_id` | path | `string` | Yes | ID of task to check |

#### Responses

**200** — Successful check of the task status

Content types: `application/json`

**Schema:** `TaskStatusResponseBodySingleUrlResultsV2`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `url` | `string` | No | URL to download this result. Valid for 2 hours |

**400** — Invalid task ID

Content types: `application/json`

**Schema:** `object`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `error` | `string` | No | — |

**401** — Invalid or missing API key

Content types: `application/json`

**Schema:** `object`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `error` | `string` | No | — |

**500** — Task execution timeout

Content types: `application/json`

**Schema:** `object`

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `status` | `integer` | No | Response status |
| `error` | `string` | No | — |
